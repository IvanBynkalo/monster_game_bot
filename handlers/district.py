...
async def district_handler(message: Message):
    ...
    if not player.current_district_slug:
        await message.answer("В этой локации пока нет доступных районов.", reply_markup=main_menu(player.location_slug))
        return
    await message.answer(render_district_card(player.location_slug, player.current_district_slug), reply_markup=main_menu(player.location_slug))

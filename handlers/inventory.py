...
async def inventory_handler(message: Message):
    ...
    await message.answer(
        ...
        reply_markup=inventory_menu(inventory, player.location_slug),
    )
...
async def use_small_potion_handler(message: Message):
    ...
    if get_item_count(message.from_user.id, "small_potion") <= 0:
        await message.answer("🧪 У тебя нет малого зелья.", reply_markup=inventory_menu(inventory, player.location_slug))
        return
    if player.hp >= player.max_hp:
        await message.answer("❤️ Здоровье уже полное.", reply_markup=inventory_menu(inventory, player.location_slug))
        return
...
async def use_big_potion_handler(message: Message):
    ...
    if get_item_count(message.from_user.id, "big_potion") <= 0:
        await message.answer("🧪 У тебя нет большого зелья.", reply_markup=inventory_menu(inventory, player.location_slug))
        return
    if player.hp >= player.max_hp:
        await message.answer("❤️ Здоровье уже полное.", reply_markup=inventory_menu(inventory, player.location_slug))
        return
...
async def use_energy_capsule_handler(message: Message):
    ...
    if get_item_count(message.from_user.id, "energy_capsule") <= 0:
        await message.answer("⚡ У тебя нет капсулы энергии.", reply_markup=inventory_menu(inventory, player.location_slug))
        return
    if player.energy >= 12:
        await message.answer("⚡ Энергия уже полная.", reply_markup=inventory_menu(inventory, player.location_slug))
        return
...
async def use_spark_tonic_handler(message: Message):
    ...
    if get_item_count(message.from_user.id, "spark_tonic") <= 0:
        await message.answer("✨ У тебя нет настоя искры.", reply_markup=inventory_menu(inventory, player.location_slug))
        return
    if player.energy >= 12:
        await message.answer("✨ Энергия уже полная.", reply_markup=inventory_menu(inventory, player.location_slug))
        return
...
async def use_field_elixir_handler(message: Message):
    ...
    if get_item_count(message.from_user.id, "field_elixir") <= 0:
        await message.answer("🌼 У тебя нет эликсира лугов.", reply_markup=inventory_menu(inventory, player.location_slug))
        return
...
async def use_crystal_focus_handler(message: Message):
    ...
    if get_item_count(message.from_user.id, "crystal_focus") <= 0:
        await message.answer("💎 У тебя нет кристального концентрата.", reply_markup=inventory_menu(inventory, player.location_slug))
        return
...
async def use_swamp_antidote_handler(message: Message):
    ...
    if get_item_count(message.from_user.id, "swamp_antidote") <= 0:
        await message.answer("🪷 У тебя нет болотного антидота.", reply_markup=inventory_menu(inventory, player.location_slug))
        return

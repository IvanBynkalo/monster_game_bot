...
from keyboards.shop_menu import item_shop_menu, monster_shop_menu, bag_shop_menu, sell_menu
...
async def city_board_handler(message: Message):
    ...
    if not player or not is_city(player.location_slug):
        ...
        return
    await _answer_with_city_image(message, "bag_market.png", render_city_board(), city_menu(player.current_district_slug))
...
async def leave_city_handler(message: Message):
    ...
    await message.answer("🚶 Ты покидаешь Сереброград через главные ворота и выходишь в Тёмный лес.", reply_markup=main_menu("dark_forest"))
...
async def city_market_handler(message: Message):
    ...
    if not player or not is_city(player.location_slug):
        ...
        return
    await _answer_with_city_image(message, "city_square.png", "🏪 Торговая лавка открыта.", item_shop_menu())
